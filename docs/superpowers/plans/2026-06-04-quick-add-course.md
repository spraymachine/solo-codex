# Quick Add Course Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a "Quick Add" tab to `CourseImportPanel` so users can create a bare course (title, URL, deadline, status) without pasting a full AI-generated plan.

**Architecture:** Add `createCourse` to the work store with the same DB + Supabase sync pattern as `saveParsedCourse`. Add a tab switcher to `CourseImportPanel` — "Quick Add" tab renders a minimal form; "Paste Plan" tab renders the existing import UI. No new files needed.

**Tech Stack:** React, Zustand, Dexie (IndexedDB via `getWorkDb`), Supabase sync via `sbCreateCourse`

---

### Task 1: Add `createCourse` to the work store

**Files:**
- Modify: `src/lib/stores/work-store.ts`

- [ ] **Step 1: Add `CourseInput` type and `createCourse` to the `WorkState` interface**

In `src/lib/stores/work-store.ts`, after the `ProjectInput` type (around line 55), add:
...
Then in the `WorkState` interface (around line 73), add after `saveParsedCourse`:

```typescript
createCourse: (input: CourseInput) => Promise<WorkCourse>;
```

Also add `CourseStatus` to the imports from `@/lib/types`:
...
- [ ] **Step 2: Implement `createCourse` in the store**

In the store implementation, after the `saveParsedCourse` implementation (around line 357), add:
...
git commit -m "feat: add createCourse action to work store"
```

---

### Task 2: Add Quick Add tab UI to `CourseImportPanel`

**Files:**
- Modify: `src/components/work/course-import-panel.tsx`

- [ ] **Step 1: Add imports and tab state**

At the top of `course-import-panel.tsx`, update the imports:
...
Inside `CourseImportPanel`, add tab state and quick-add form state at the top of the function body (after the existing state declarations):
...
- [ ] **Step 2: Add `handleQuickAdd` handler**

Inside `CourseImportPanel`, after the existing `saveCourse` function, add:
...
- [ ] **Step 3: Replace the panel header with a tab switcher**

Replace the existing panel header block:
...
- [ ] **Step 4: Wrap existing panel body in conditional, add Quick Add body**

Replace the existing `<div className="space-y-4 p-5">` block (everything inside the panel after the header) with:
...
```bash
git add src/components/work/course-import-panel.tsx
git commit -m "feat: add Quick Add tab to CourseImportPanel"
```
